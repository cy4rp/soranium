...
import { efToRaw, parseTx } from '../tx.js'
...
export const listUtxos = (opts: {
  ...
  const sql = `SELECT u.*, t.raw_hex FROM wallet_utxos u JOIN wallet_txs t USING (txid)
    ${where.length ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY u.created_at, u.txid, u.vout${limit}`
  return db.prepare(sql).all(...args).map((row) => rowToUtxo(row as unknown as Record<string, unknown>))
}
...
export const recordBroadcast = (built: BuiltTx, inputs: SpendInput[]): { txid: string; added: number } => {
  const imported = importTx(built.rawHex)
  markSpent(inputs, imported.txid)
  return imported
}
